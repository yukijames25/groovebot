...
- **JointCommand 橋渡し v1.2 プリミティブ拍位相の規約化**（2026-06-21、§14 v1.2 phase 仕様節）: v1.1 ナレーションが `headbang` ピークを `b+0.48 (off-beat)` と honest に暴いていた事実を起点に、プリミティブ閉形式の位相を以下の規約へ統一: (A) **accent 系** (headbang / bob_nod / fist_pump) は `(1+cos 2πb)/2` で**ピークを各拍 on-beat に**(b=0,1,2,...)、(B) **clap** は `(1-cos πb)/2` の 2 拍周期で**b=1,3,5,...(楽器的 2 & 4 拍 = backbeat) にアクセント**(b=0,2 は rest)、(C) **連続系** (sway / rock / penlight_wave) は `sin(πb)` を**据え置き**(整数拍ゼロ通過＆半拍で ±最大変位、"in-the-pocket" sway 系の自然な感触、narrate は `off-beat` と honest 報告)、(D) **quiet_listen** は据え置き。振幅・SOFT_AMP・intensity スケーリング・URDF クランプ・公開 IF (`GrooveContext`/`JointCommand`/`StyleGrooveGenerator`/`metronome_from_style`) いずれも不変。`MOVE_PRIMITIVES` 近傍に拍位相規約をコメント表として明文化。検証: 8 move × 4 拍 × 50 Hz の narrate サンプル(`narrate(...verbose=True)`)で accent 3 種は全 beat 行で `peak=... @ b+0.96 (on-beat)`、clap は 0/2 拍で `b+0.96` (rising into backbeat)・1/3 拍で `b+0.00` (peak AT backbeat) の位相が交互に出る指紋、sway/rock は `b+0.44 (off-beat)` の交互正負ピーク、quiet_listen は ±0.04 rad の極小値。テスト: `tests/test_groove_style.py` (+7 件、56 件; 各 move の意図位相を `prim(b_int, 1.0)` で直接アサート＋クラップ 4 ポイントの rest/accent/period)、`tests/test_narrate.py` (+4 件、28 件; accent 3 種を parametrize で全 beat on-beat、clap は peak_phase が 0.10/0.90 境界で交互)。`narrate.py` の `_beat_phase_label` docstring に「accent peak は on-beat、continuous は仕様通り off-beat ラベル」と書き直し。docs/SYSTEM_SPEC.md §14 v1.2 に move × phase 規約表 ＋ サンプル narrate を追記。pytest **432 passed / 2 skipped**(+11 件)。正直な限界: 拍源は依然 `MetronomePerception` (BPM ドリブン) で、実歌唱のタイミング揺らぎには未追従(M2 で `ReferenceAligner` に差替予定)。
