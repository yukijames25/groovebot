...
### GrooveStyleSelector v3.1: affect 統合 (DEAM) + V/A → mood
動機: v3 で DEAM の arousal/valence を学習済み(test R²=0.522/0.451, r=0.723/0.675)。v3 時点では heuristic がデフォルトで learned は opt-in だったが、PANNs 利用可能なパイプラインでは learned を既定にしないと「ヒューリスティック r=0.42 vs learned r=0.72」のギャップが実運用で出ない。同時に DEAM valence が手に入ったことで、circumplex (Russell 1980) で mood を VA から導く経路が成立し、MTG mood ヘッドの calm sink 問題 (sad→calm 62%) を回避できる可能性が出てきた。

#### 既定パスの見直し(`select.py`)
| selector 構築                                                | arousal 源           | valence 源           |
|--------------------------------------------------------------|----------------------|----------------------|
...
| `... arousal_fn=...` / `... valence_fn=...`                  | 明示オーバライド     | 明示オーバライド     |

PANNs backbone embed は分類ヘッドと回帰ヘッドで**共有**(1 embed / `select()` 呼び出し)。`tests/test_select_affect_default.py::test_embedding_shared_across_head_and_regression_head` で固定済(M2 リアルタイムループの予算)。`GrooveStyle` public 出力契約は不変。

#### V/A → mood (`groovebot/style/mood_from_va.py`)
circumplex の 4 象限が 6 mood のうち 4 つに綺麗に対応する:

| V (valence) | A (arousal) | quadrant mood |
|------------:|------------:|:--------------|
|         1.0 |         1.0 | happy         |
|         0.0 |         1.0 | aggressive    |
|         1.0 |         0.0 | calm          |
...
**epic と dark は純 V/A 座標ではない**:
- **epic**: 高 arousal + valence 曖昧(triumphant=positive vs ominous-grand=negative)。MTG v3 ヘッドが裏付け: epic→aggressive 8% / aggressive→epic 0% で V/A 空間上での重なりはあるが分離軸は別(grandness / cinematic 度合い)。
- **dark**: 低〜中 arousal + 低 valence で sad と重なる。MTG v3 ヘッドは timbre (dark = brassy/horror, sad = acoustic/intimate) で分離している。

既定の `DEFAULT_QUADRANT_PROTOTYPES` は 4 象限のみ。epic/dark は確率 0。opt-in する場合は `PROTOTYPES_WITH_AUX` (草案: `epic=(V=0.55, A=0.95, w=0.5)`, `dark=(V=0.10, A=0.30, w=0.5)`) を `mood_va_prototypes` に渡す。weight=0.5 の under-weight は 4 象限の corner mass を盗まないため。これらは草案で、ユーザが listening test で調整する想定。

membership は **soft**(Gaussian on squared distance, sigma=0.45)で argmax ではない。中心 (0.5, 0.5) では 4 象限へほぼ一様。出力は確率和 1.0 で `table.select_move` の mood soft 入力にそのまま流せる。

#### mood source 切替(`mood_source`)
- `mood_source="head"` (既定): MTG-trained `StyleHead` mood ヘッドの softmax(現行動作と同一)。
- `mood_source="va"`: arousal/valence から `mood_probs_from_va` で導出。`regression_head` か明示 `valence_fn` 必須(構築時 ValueError でガード)。

MTG head はメモリに残し続け、backbone embed もそのまま供給される(genre_probs のため)ので切替コストはゼロ。両経路を並列で走らせて比較する用途に最適。

#### MTG mood vs DEAM V/A-mood 比較 (`experiments/compare_va_mood_vs_mtg.py`)
同じクリップ集合に対して両 pipeline を流し、per-clip 予測 CSV と `report.json` を吐く:
- `agreement_rate`(MTG argmax == VA argmax の割合)
- `confusion_matrix_mtg_rows_va_cols`(6×6)
- `per_class_profile_by_mtg_mood`(MTG ヘッド各クラスについて DEAM (a, v) の mean/std) — **MTG 退役診断の本体**: MTG `calm` クリップが V/A の calm 象限にクラスタすれば V/A は MTG を再現できる、散らばれば MTG `calm` は他クラスを吸っている証拠。
- `calm_sad_stability`(`{calm, sad}^2` subset — v3 報告の sad→calm 62% 崩壊が VA で解けるか)
- `accuracy_vs_gt`(manifest の `mood_class` 列が GT として使える時)

`--include-aux-moods` で epic/dark を VA 側で有効化。既定は 4 quadrant のみ(quadrant story の単独評価)。

#### MTG mood ヘッド退役判定基準
1. 同 holdout で V/A `accuracy_vs_gt` ≥ MTG `accuracy_vs_gt`(綺麗な 4 象限 story が学習ヘッドに勝つか同等)。
2. 手付け subset で `calm_sad_stability.agreement_rate` ≥ 0.8(VA 経路で sad→calm 崩壊が止まる)。
3. genre ごと 1 クリップ以上で listening test 合格(VA 経路が音楽的に妥当な mood を返す)。

1+2 が立ち 3 が通れば、次リビジョンで MTG mood ヘッドを v3 学習パイプラインから削り(回帰ヘッド + V/A map で代替)、`style/` モジュール表面を縮める。

#### 限界 (v3.1)
- **V/A → mood の妥当性は 4 象限まで**。epic/dark は V/A だけでは決まらない(草案座標を持たせたが、本来は learned classifier か timbre 特徴量を追加すべき)。
- **soft sigma=0.45 はチューンしていない**。中心が ~uniform / corner が dominant という性質しか確認していない。listening test で締めるべき。
- **比較レポートは実データ用 ckpt が必要**。`--synthetic-stub` で wiring は確認できるが、退役判断は実 ckpt + 実音源での実行を要する。CI では stub のみ。

---

### JointCommand橋渡し v1: スタイル条件付きグルーヴ生成 → MuJoCo sim
動機: 検証済み属性(style/arousal/tempo)を関節動作に変換し「音声→ノり方→動き」をsimで可視化。実機不要(M1のMuJoCo backendが対象)。
- StyleGrooveGenerator: GrooveStyle(種類＋強さ)＋拍位相 → JointCommand。
  プリミティブ: headbang=首pitch振動 / bob=首nod / sway=体幹roll / rock=体幹pitch / fist-pump=肩pitch / clap=両腕2&4 / arm-wave=腕左右 / still=最小。
  変調: 振幅・速さ∝arousal、位相・レート∝拍。全関節URDF上限クランプ(NFR-4)。
- 拍源(暫定): 属性tempo(BPM)からメトロノーム位相(本物の整合=DAMPトラックは保留)。差し替え可。
- 配線: GrooveStyleSelector → Orchestrator(30-50Hz) → StyleGrooveGenerator → JointCommand → MuJoCo → 動画出力。公開IF不変。
正直な限界: simは頭脳→動作ロジック/sim見た目/関節安全のみ検証。実サーボ動特性・実機見た目は実機待ち。プリミティブは手作り(学習はM3)。拍は暫定tempo駆動(本物はM2)。

#### v1 実装(2026-06-17)
モジュール:
- `groovebot/groove_style.py` — `StyleGrooveGenerator(GrooveGenerator)` ＋ プリミティブ表 `MOVE_PRIMITIVES` (8 種、`table.MOVES` と網羅一致) ＋ `SOFT_AMP`(URDF 内側に取った設計上のソフト上限) ＋ `metronome_from_style(style)`(`style.tempo_bpm` 駆動の `MetronomePerception` ビルダ)。
- `experiments/render_groove.py` — MuJoCo backend で動かして per-tick CSV ＋ PIL 製 GIF を `data/renders/<tag>.{csv,gif}` に出力する開発ツール。`--all-moves`(全プリミティブ別ファイル)／`--style MOVE`(強制)／`--audio PATH`(セレクタ経由)／`--skip-render`(GL 無し環境)。

設計の決まり:
- **プリミティブは `(beat_pos, intensity) -> dict[str, float]`**。`intensity=0` で必ず neutral pose に戻る(リファクタで全プリミティブ全項に intensity を掛けた)。返すのは触る関節だけ ＋ generator が残りを `neutral_pose()` で埋めるので、毎 tick の `JointCommand.targets` は `JOINT_NAMES` を完全に覆う。
- **URDF クランプは二重ガード**。`SOFT_AMP` が設計上の振幅天井(URDF 内側に 5–15 % 余裕)、Orchestrator の `clamp_command` が NFR-4 のハード安全。テスト `test_primitive_outputs_respect_urdf_limits` が全プリミティブ × intensity 5 段階 × 4 拍 = 約 8,000 サンプルで URDF 範囲を sweep する。
- **GrooveContext は不変**。`set_style(style)` で generator が直接スタイルを受ける(セレクタは秒オーダ、ループは 30–50 Hz、結合は粗)。`use_ctx_arousal=True` を opt-in にすれば `ctx.arousal` でリアルタイムに振幅変調(M2 で arousal estimator が入った時用)。
- **拍源は `metronome_from_style`** で v1 限定の暫定。`style.tempo_bpm` が 0 等の縮退値でも `floor_bpm=40` でループが止まらないようにガード。M2 で `ReferenceAligner` に差し替わってもこのファイルは無関係。

#### v1.2 拍位相の契約(2026-06-21)
各プリミティブが「どの拍位相でピーク/アクセントを置くか」は **MOVE_PRIMITIVES 近傍にコメント表として明文化**し、`tests/test_groove_style.py` でアサート固定する。v1 当初は accent 系/clap がそれぞれ b+0.5 / 各拍 のピーク位相になっており、narrate (v1.1) が `on-beat` 境界 10% ルールで「off-beat」と honest に報告していた。これを以下の規約に統一した。

| move          | category   | cycle  | peak / accent lands at                              | 主関節             |
|---------------|------------|--------|-----------------------------------------------------|--------------------|
| headbang      | accent     | 1 beat | 各拍(`b = 0, 1, 2, ...`)に最深 dip                  | neck_pitch         |
| bob_nod       | accent     | 1 beat | 各拍にピーク nod                                    | neck_pitch         |
| fist_pump     | accent     | 1 beat | 各拍に肩pitch最深(腕高)＋肘最屈                     | l/r_shoulder_pitch |
| clap          | accent     | 2 beat | 楽器的拍 2 & 4(`b = 1, 3, 5, ...` = backbeat)       | l/r_shoulder_roll  |
| sway          | continuous | 2 beat | 各拍でゼロ通過、半拍で最大変位(`sin(πb)`)           | torso_roll         |
| rock          | continuous | 2 beat | sway と同位相規約                                   | torso_pitch        |
| penlight_wave | continuous | 2 beat | sway と同位相規約(wave 成分 = neck_yaw, torso_roll) | l_shoulder_pitch   |
| quiet_listen  | minimal    | 4 beat | アクセント無し(呼吸のみ)                            | neck_pitch         |

実装メモ:
- アクセント系の閉形式は `(1 + cos(2π·b))/2` で「位相 0 でピーク」に固定(以前の `(1 - cos)` から反転)。
- clap は `(1 - cos(π·b))/2` で 2 拍周期＆位相 1 でピーク(backbeat)。`b=0, 2` は rest、`b=1, 3` で l_shoulder_roll が 1.0→1.6、l_elbow が 1.2→1.8 まで上昇。
- 連続系(sway/rock/penlight_wave)は `sin(π·b)` を据え置き。整数拍で中心通過、半拍で最大変位。"in-the-pocket" sway 系の自然な感触であり、accent 系と意図的に対立する規約。narrate が `off-beat` ラベルを返すのは仕様通り。
- 振幅・SOFT_AMP・intensity スケーリング・URDF クランプ・public IF は不変。

narrate サンプル(0-2 s, 50 Hz, 120 BPM, intensity 0.85):
```
beat#0 [t=0.000s] headbang:      neck_pitch       peak=-0.468 rad @ b+0.96 (on-beat)
beat#1 [t=0.500s] bob_nod:       neck_pitch       peak=-0.212 rad @ b+0.96 (on-beat)
beat#0 [t=0.000s] fist_pump:     l_shoulder_pitch peak=-1.615 rad @ b+0.96 (on-beat)
beat#0 [t=0.000s] clap:          l_shoulder_roll  peak=+1.360 rad @ b+0.96 (on-beat)
beat#1 [t=0.500s] clap:          l_shoulder_roll  peak=+1.358 rad @ b+0.00 (on-beat)
beat#0 [t=0.000s] sway:          torso_roll       peak=+0.297 rad @ b+0.44 (off-beat)
beat#1 [t=0.500s] sway:          torso_roll       peak=-0.297 rad @ b+0.44 (off-beat)
beat#0 [t=0.000s] rock:          torso_pitch      peak=+0.254 rad @ b+0.44 (off-beat)
```
clap が 0-bucket では b+0.96 (rising)、1-bucket では b+0.00 (peak AT beat 2)、と位相が交互に切り替わるのが backbeat の指紋。

テスト(`tests/test_groove_style.py` 56 件 + `tests/test_render_groove.py` 3 件):
- 語彙網羅: `MOVE_PRIMITIVES.keys() == MOVES`、neutral_pose は `JOINT_NAMES` 全網羅。
- 安全性: 全プリミティブ × URDF 範囲(soft amp は別アサート)。
- 単調性: intensity 0.3→0.6→1.0 で peak-to-peak が単調増(headbang/bob_nod/sway/rock)。
- 拍位相(v1.2 規約): headbang/bob_nod/fist_pump は整数拍で最深ピーク／半拍で 0 復帰、clap は b=1, 3 で l_shoulder_roll=1.6・l_elbow=1.8 ＆ b=0, 2 で rest(1.0/1.2)、sway/rock/penlight_wave は整数拍でゼロ通過＆半拍で ±最大。headbang は 4 拍窓で内部局所最小=3(b=1,2,3)、sway は 3 ゼロ交差。
- 配線: 未スタイルで neutral、未知ムーブで neutral、`set_style` の途中差し替えが次 tick から効く、Orchestrator が `StyleGrooveGenerator` を 50 Hz で 50 tick 駆動。
- 暫定拍源: `metronome_from_style(style)` の `bpm/energy/arousal` が `GrooveContext.tempo` まで通っているか、退化 BPM で floor が効くか。
- narrate(`tests/test_narrate.py` 28 件): accent 系 3 種は全 beat で `on-beat` ラベル、clap は 4 拍トレースの peak_phase が交互に `>0.90` / `<0.10` (backbeat 指紋)、sway は `off-beat`(連続系の honest report)。
- render_groove はモジュール import ＋ `_forced_style` ＋ `_moves_to_render(--all-moves)` のスモークのみ(GIF 生成は CI で叩かない)。

正直な限界(再掲):
- **sim ≠ 実サーボ**。MuJoCo backend は `smoothing=0.35` の一階低域通過で「サーボ風」に角度補間しているだけ。PD ・トルク制限・整定時間は別物。相対動作は忠実、絶対の手触りは実機で変わる。
- **拍は BPM メトロノーム**。`style.tempo_bpm` で位相を進めるだけで、ユーザの実歌唱タイミングには未追従。M2 で `ReferenceAligner` を Perception スロットに差せば generator はそのまま `ctx.beat_pos` を拾う。
- **プリミティブは手作り**。閉形式の正弦/余弦で、学習動作ではない。M3 で codebook + Transformer 生成器に差し替わるとここは fallback 側へ。
- **render_groove は ckpt なしでもセレクタを構築する**(ヘッド重みランダム)。出力 `GrooveStyle` は table 経由で必ず legal だが、genre/mood は意味を持たない(視覚化用途)。本気の自動選曲は `GrooveStyleSelector.from_panns(...)` に PANNs CNN14 + 学習済ヘッドを渡す。

#### v1.1 ナレーション層(2026-06-17、観測性)
動機: v1 の本番出力は `JointCommand` のままだが、研究/デバッグ/卒研指導の場面で「セレクタが何を読んで、なぜそのムーブを選び、結果どの関節がどう動いたか」を**1 行/1 段落の自然文**で出せると価値が大きい。CSV を眺める代わりにそのまま会話/ログ/レポートに貼れる。実機展開前の "プレースホルダ・ナレーション"(まだ動かない身体の代わりに語る) としても再利用可能。

モジュール: `groovebot/style/narrate.py`
- `format_window_summary(style, *, t_start, t_end | seconds, rate, reason) -> str`
  「[t_start-t_end s, NN Hz] 知覚 → 判断 → 動作」の 3 行段落。`reason` 既定値は `genre × arousal_bucket × top_mood(prob)`、明示渡しで上書き(retrieval/aggregator 経由の説明に差し替え可)。
- `format_beat_trace(style, *, commands, rate, bpm, max_beats) -> list[str]`
  per-tick `JointCommand.targets`(`render_groove` が CSV へ落としているもの) を拍 index でバケットし、主関節のピーク角度・拍内位相 (`b+0.50` 等) と on-beat/off-beat ラベルを 1 拍 1 行で返す。境界 10% を on-beat とするだけの宣言的ラベルで、headbang のような **b+0.5 ピーク=off-beat** という事実を歪めず報告する(隠さない)。
- `narrate(style, commands=None, *, rate, seconds, verbose, max_beats) -> str`
  window summary のみ／verbose=True で beat trace を末尾連結。`commands`/`rate` 未指定の verbose は省略ノート 1 行で fallback(呼び出し側で gating 不要)。
- 静的表:
  - `PRIMARY_JOINTS`(8 ムーブ各 1-4 関節、`MOVE_PRIMITIVES` が実際に駆動する関節と一致、テストで固定)
  - `CYCLE_BEATS`(1/2/4 拍/サイクル、説明文用)
  - `MOVE_DESCRIPTOR`(日本語短文)
  - `SOFT_AMP` を流用して「設計上限 |θ|≈X.XX rad」表示

`render_groove.py` 連携:
- `--narrate` で window summary を stdout に印字、`--verbose` で beat trace も追加。両者は `--skip-render` と直交で、`--skip-render --narrate` は「音声→ナレーションのみ(GIF 無し)」の内部検証パス(CI / 開発機どちらでも通る)。
- `--narrate-max-beats N` で長尺ランの beat trace を頭 N 拍に切る。

設計上の決まり(spec 不変条件と整合):
- **公開 IF 不変**。`narrate` モジュールは `GrooveContext` / `JointCommand` / `RobotBackend` を import するだけで、これらを書き換えない。生成側(`StyleGrooveGenerator`)・制御ループ(`Orchestrator`)とは入出力上**直交**。
- **新しい内部状態を持たない**。引数の `GrooveStyle` と `commands`(per-tick targets) は両方とも v1 で既に CSV/メモリに存在しているもの。`narrate` はそれを文字列化する pure function。
- **本番制御ループから呼ばない**(NFR-7 への影響を避ける)。観測/デバッグ専用。M2 で ReferenceAligner が入った後も、リアルタイム経路ではなく run 終了後 / 別スレッドからのバッチ呼び出し想定。

テスト(`tests/test_narrate.py`、24 件): 語彙網羅(`PRIMARY_JOINTS.keys() == MOVES`)、主関節がプリミティブ出力に実在(`MOVE_PRIMITIVES[move](0.25, 1.0)` の含有確認)、window summary の必須フィールド(move/intensity/tempo/genre/mood/bucket/arousal/SOFT_AMP)、未登録ムーブの安全な「未登録」表記、上書き reason の通過、beat trace の (a) 1 拍 1 行 (b) `max_beats` 切り (c) ピーク値が URDF 範囲内 (d) 空 commands と bpm=0 のグレースフル fallback (e) 明示 bpm 上書き (f) sway の off-beat ラベリング(b+0.5 ピーク=off-beat の honest report)、`narrate()` の summary-only/verbose/未指定 commands 各経路。

正直な限界(v1.1 narrate):
- **記述は表ベース**。`MOVE_DESCRIPTOR` / `PRIMARY_JOINTS` は手作りで、プリミティブ実装の真の主関節は `MOVE_PRIMITIVES[move](0.25, 1.0)` の中身。テストで一致を固定しているが、新規プリミティブを足す時は narrate 表も同時更新が必要(`test_narration_tables_cover_full_move_vocabulary` が抜けを検出)。
- **本番ループには入れない**。文字列フォーマットは桁の Python のスループット内に十分収まるが、Orchestrator(30-50 Hz)の予算は M2 知覚側で消費する前提なので、リアルタイム経路に紛れ込ませない方針を spec として明示。
- **on-beat / off-beat は宣言的ラベル**。境界 10% という閾値が出した「v1 primitive の peak が off-beat になる」は **正しい報告**であり修飾しない(ナレーション側で誤魔化さない、プリミティブ位相を変えたければプリミティブ側を直す)。

---

### JointCommand橋渡し v1.1: テキスト・ナレーション(観測性)
動機: 実機が無く動作の物理検証ができないため、内部状態(属性→スタイル→動作)を人間可読テキストで出力し、判断と動きの妥当性を言葉で確認。デバッグ＋デモ＋実機前プレースホルダを兼ねる。
- narrate(): 内部状態 → 人間可読文字列。
  - 窓サマリ(スタイル決定時): genre/mood/arousal/tempo → GrooveStyle(種類＋強度)＋理由(表規則/soft-blend) → 動作概要(プリミティブ・主関節・拍との関係)。
  - 拍トレース(--verbose任意): 拍ごとに primitive段階・主関節角・amp・on/off-beat。
- 配線: render_groove に --narrate/--verbose。GIF無しの「音声→ナレーションのみ」内部検証経路も。
- 公開IF不変。ナレーションは観測層、本番出力は JointCommand のまま。